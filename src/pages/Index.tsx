...
const Index = () => {
  ...
  const [highlightSingletonChat, setHighlightSingletonChat] = useState(false);
  const [chatInstances, setChatInstances] = useState<
    Record<string, ChatNonSingleton>
  >({});
  ...
  const handleOpenChat = () => {
    if (isSingleton) {
      if (!singletonOpen) {
        ...
        setHighlightSingletonChat(true);
        setTimeout(() => setHighlightSingletonChat(false), 500);
      }
    } else {
      // For non-singleton mode, create a new chat window with unique ID
      const newWindowId = `chat_${Date.now()}`;
      setChatWindows((prev) => [...prev, newWindowId]);

      // Tạo instance mới của ChatNonSingleton cho mỗi cửa sổ
      setChatInstances((prev) => ({
        ...prev,
        [newWindowId]: new ChatNonSingleton(newWindowId),
      }));
    }
  };
        ...
        <div className="glass-panel p-8 text-center space-y-6">
            ...
            <div className="text-sm text-muted-foreground">
              {isSingleton ? (
                <>
                  Current Status:{" "}
                  {singletonOpen ? (
                    <span className="text-primary">Singleton Chat Open</span>
                  ) : (
                    "No Active Chat"
                  )}
                </>
                ...
                <>
                  ...
                  <span className="text-destructive">
                    {chatWindows.length} Chat Windows Open
                  </span>
                </>
              )}
            </div>
          ...
          <div className="pt-4 space-y-2 text-sm text-left">
            ...
            <ul className="list-disc pl-5 text-muted-foreground space-y-1">
              ...
              <li>
                In chat applications, it prevents duplicate instances and data
                loss
              </li>
      ...
      <div className="fixed bottom-8 right-8 flex flex-col items-end space-y-4 z-20">
        {isSingleton && singletonOpen && (
          <ChatWindow
            ...
            highlight={highlightSingletonChat}
          />
        )}
